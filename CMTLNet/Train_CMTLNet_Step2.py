...
import numpy as np
import pandas as pd
from model.CMTLNet import CMTLNet
from torch.nn import CrossEntropyLoss
from torch.utils.data import DataLoader
from torch.optim.lr_scheduler import ReduceLROnPlateau
from dataLoder.loader_S2 import TrainDataLoader_balance as TrainDataLoader
from dataLoder.loader_S2 import TestDataLoader
...
def reshape_input_data(data):
    ...
    Parameters:
        data (tuple): Tuple containing images, classification labels.

    Returns:
        tuple: Reshaped images, classification labels.
    """
    images, labels = data
    # Reshape images
    images = images.view(
...
def train(fold, pkl_paths):
    ...
    for epoch in range(config.epochs_s2):
        ...
        for batch_idx in range(len(train_loader_1p19q)):
            ...
            # Reshape input data
            imgs_1p19q, label_1p19q = reshape_input_data(data_1p19q)
            imgs_IDH, label_IDH = reshape_input_data(data_IDH)
            imgs_LHG, label_LHG = reshape_input_data(data_LHG)
            ...
            condition_1p19q = torch.tensor(0, dtype=torch.long,
                                               device=label_1p19q.device).repeat(1, label_1p19q.size(0))[0]
            condition_IDH = torch.tensor(1, dtype=torch.long,
                                               device=label_IDH.device).repeat(1, label_IDH.size(0))[0]
            condition_LHG = torch.tensor(2, dtype=torch.long,
                                               device=label_LHG.device).repeat(1, label_LHG.size(0))[0]

            condition = torch.cat([condition_1p19q, condition_IDH, condition_LHG], dim=0)

            # Forward pass through the model
            (out_cls_list_UFE, out_cls_labels_UFE,
             out_cls_list_UCFC, out_cls_labels_UCFC) = model(combined_input, label_list, condition)

            # Compute losses
            loss_cls_U1 = cross_entropy_loss(out_cls_list_UFE[0], out_cls_labels_UFE[0])
            loss_cls_U2 = cross_entropy_loss(out_cls_list_UFE[1], out_cls_labels_UFE[1])
            loss_cls_U3 = cross_entropy_loss(out_cls_list_UFE[2], out_cls_labels_UFE[2])

            loss_cls_1 = cross_entropy_loss(out_cls_list_UCFC[0], out_cls_labels_UCFC[0])
            loss_cls_2 = cross_entropy_loss(out_cls_list_UCFC[1], out_cls_labels_UCFC[1])
            loss_cls_3 = cross_entropy_loss(out_cls_list_UCFC[2], out_cls_labels_UCFC[2])

            # Total classification loss
            total_loss = loss_cls_U1 + loss_cls_U2 + loss_cls_U3 + loss_cls_1 + loss_cls_2 + loss_cls_3
...
if __name__ == '__main__':
    ...
    for fold in range(5):
        model = CMTLNet(in_channels=3, num_classes=2, Arcm=0.5, multi=3)  # Initialize the model
        model_basic_CFE = torch.load(Base_CEF_model_paths[fold])
        model.ShareTask.load_state_dict(model_basic_CFE.state_dict())
